...
class _001TwoSum {
    ...
    companion object{
        ...
        fun main(args: Array<String>) {
            val _solver  = _001TwoSum()
            println(
                "Result: " +
                        toJsonString(_solver.twoSum(intArrayOf(3, 3), 6))
            )
            // Example 1
            println(
                "Result for [2, 7, 11, 15], target 9: " +
                        toJsonString(_solver.twoSumOptimized(intArrayOf(2, 7, 11, 15), 9))
            )

            // Example 2 (the edge case from your original code)
            println(
                "Result for [3, 3], target 6: " +
                        toJsonString(_solver.twoSumOptimized(intArrayOf(3, 3), 6))
            )

            // Example 3
            println(
                "Result for [3, 2, 4], target 6: " +
                        toJsonString(_solver.twoSumOptimized(intArrayOf(3, 2, 4), 6))
            )
        }
    }
}
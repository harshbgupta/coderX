...
fun main() {
    ...
    val target1 = 8
    println("Input: nums=${nums1.contentToString()}, target=$target1, Output: ${solutionWithLogNTimeComplexity(nums1, target1)}") // Expected: 4

    // Standard case: Target is in the left half (before the pivot)
    val nums2 = intArrayOf(8, 9, 10, 11, 12, 15, 0, 1)
    val target2 = 15
    println("Input: nums=${nums2.contentToString()}, target=$target2, Output: ${solutionWithLogNTimeComplexity(nums2, target2)}") // Expected: 5

    // Target not found
    val nums3 = intArrayOf(4, 5, 6, 7, 0, 1, 2)
    val target3 = 3
    println("Input: nums=${nums3.contentToString()}, target=$target3, Output: ${solutionWithLogNTimeComplexity(nums3, target3)}") // Expected: -1

    // Edge case: No rotation (a standard sorted array)
    val nums4 = intArrayOf(1, 2, 3, 4, 5, 6)
    val target4 = 4
    println("Input: nums=${nums4.contentToString()}, target=$target4, Output: ${solutionWithLogNTimeComplexity(nums4, target4)}") // Expected: 3

    // Edge case: Target is the first element
    val nums5 = intArrayOf(5, 1, 3)
    val target5 = 5
    println("Input: nums=${nums5.contentToString()}, target=$target5, Output: ${solutionWithLogNTimeComplexity(nums5, target5)}") // Expected: 0

    // Edge case: Single element array, target found
    val nums6 = intArrayOf(1)
    val target6 = 1
    println("Input: nums=${nums6.contentToString()}, target=$target6, Output: ${solutionWithLogNTimeComplexity(nums6, target6)}") // Expected: 0

    // Edge case: Single element array, target not found
    val nums7 = intArrayOf(1)
    val target7 = 0
    println("Input: nums=${nums7.contentToString()}, target=$target7, Output: ${solutionWithLogNTimeComplexity(nums7, target7)}") // Expected: -1

    // Edge case: Empty array
    val nums8 = intArrayOf()
    val target8 = 5
    println("Input: nums=${nums8.contentToString()}, target=$target8, Output: ${solutionWithLogNTimeComplexity(nums8, target8)}") // Expected: -1
}
